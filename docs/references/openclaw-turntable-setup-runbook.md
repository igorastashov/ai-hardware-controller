...
- На Machine B подтвержден plugin load + enabled + `tools.alsoAllow` для `turntable_*`.
...
- `Tool not available`: проверь `plugins.entries.turntable.enabled` и `tools.alsoAllow` на нужном агенте.
...
- `Tool turntable_state not found` при `plugins info turntable = loaded`: обычно это политика конфигурации (`profile=minimal` + plugin-only `tools.allow`). Переключите агент на `tools.alsoAllow` для `turntable_*`, затем перезапустите gateway.
